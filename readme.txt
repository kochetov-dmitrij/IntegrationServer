...
Prerequisites. Other versions of packages most probably are not compatible with each other. Complete installation script for ubuntu is at the bottom of the doc.
 ...
 - ansible 2.9
Steps:
 - Inside this folder run `vagrant up`. It launches a VirtualBox VM using Vagrant and provisions it with Ansible playbook
Test cases for the final state:
 - Vagrant and ansible did not report any error.
 - GitLab http://192.168.11.10/gitlab/devopser/DevopsCalculator can be opened and logged in as name/pass = devopser/f2qskL4Zd6
   The project must have 'master' and 'ready/init' branches.
 - Artifactory http://localhost:11003/ui/repos/tree/General/generic-local can be opened and logged in as name/pass = admin/sHMHY6iZjh
 - Jenkins http://localhost:11001/blue/organizations/jenkins/DevopsCalculator/activity/ can be opened and logged in as name/pass = devopser/cLVc1VH73D
 - Inside Jenkins wait until the first build from 'ready/init' reaches 'Push to master' stage. http://localhost:15001 (stage server) must display the app.
 - On the page with 'Push to master' stage press 'Proceed'. A new commit will be pushed to 'master'. 'ready/init' branche will be deleted.
 - Inside Jenkins wait until the first build from 'master' completes. http://localhost:15900 (prod server) must display the app.
